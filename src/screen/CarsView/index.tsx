...
const CarsView: React.FC = () => {
  ...
  async function fetchCars() {
    try {
      ...
      setErrorMessage(
        "Erro ao buscar os veículos. Por favor, tente novamente."
      ); // Define a mensagem de erro para exibir na interface
    }
  }
  ...
  const handleDeleteCar = async (carID: number) => {
    try {
      const response = await fetch(
        `http://192.168.0.11:5000/veiculos/${carID}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        console.error(
          "Falhou a tentativa de deletar o veículo:",
          response.statusText
        );
      }
    } catch (error) {
  ...
  const handleEditCarWrapper = async (updatedCar: Veiculo) => {
    try {
      const response = await fetch(
        `http://192.168.0.11:5000/veiculos/${updatedCar.id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <View style={styles.container}>
      ...
      {errorMessage ? (
        ...
        <ScrollView style={styles.scrollRenderCars}>
          <View style={styles.renderCars}>{renderCars()}</View>
        </ScrollView>
      )}
    </View>
  );
